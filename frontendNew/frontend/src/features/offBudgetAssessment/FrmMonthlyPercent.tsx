...
const FrmMonthlyPercent: React.FC = () => {
    ...
    try {
      ...
      for (const officer of response.data) {
        ...
        if (recofficerResponse.data.totsum && recofficerResponse.data.totsum > 0) {
          const varValueOfBillsDistributed = recofficerResponse.data.totsum!;
          const varBudgetValue = varValueOfBillsDistributed / 12;
          const varPercent = (recofficerResponse.data.totsum! / varBudgetValue) * 100;
  ...
  return (
    <div style={{ backgroundColor: '#FFC0C0', height: '100vh', width: '100vw', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px' }}>
      ...
      <h2 style={{ textDecoration: 'underline', color: '#400000', fontSize: '20px' }}>Collector Monthly Performance Trend (Percentage)</h2>
