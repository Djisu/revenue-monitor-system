...
router.post('/', async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT * FROM tb_BussCurrBalance WHERE buss_no = $1 AND fiscalyear = $2', [bussCurrBalanceData.buss_no, bussCurrBalanceData.fiscalyear]);
        if (result.rows.length > 0) {
            res.status(404).json({ message: 'BussCurrBalance record exists' });
            return;
        }
        // Insert the new BussCurrBalance data
        await pool.query(`INSERT INTO tb_BussCurrBalance (buss_no, fiscalyear, balancebf, current_balance, totalAmountDue, transdate, electoralarea) 
...
router.get('/', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM tb_BussCurrBalance');
...
router.get('/:buss_no/:fiscalyear', async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT * FROM tb_BussCurrBalance WHERE buss_no = $1 AND fiscalyear = $2', [buss_no, fiscalyear]);
        if (result.rows.length > 0) {
...
router.put('/:buss_no/:fiscalyear', async (req, res) => {
    ...
    try {
        ...
        await pool.query(`UPDATE tb_BussCurrBalance SET fiscalyear = $1, balancebf = $2, current_balance = $3, totalAmountDue = $4, 
...
router.delete('/:buss_no/:fiscalyear', async (req, res) => {
    const { buss_no, fiscalyear } = req.params;
    try {
        const result = await pool.query('SELECT * FROM tb_BussCurrBalance WHERE buss_no = $1 AND fiscalyear = ?', [buss_no, fiscalyear]);
        if (result.rows.length === 0) {
            res.status(404).json({ message: 'BussCurrBalance record does not exist' });
            return;
        }
        // Delete the BussCurrBalance record
        await pool.query('DELETE FROM tb_BussCurrBalance WHERE buss_no = $1 AND fiscalyear = $2', [buss_no, fiscalyear]);
