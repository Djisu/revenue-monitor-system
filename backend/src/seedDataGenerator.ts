...
export const generateBusinessData = (startNumber: number, count: number): BusinessData[] => {
    ...
    for (let i = startNumber; i < startNumber + count; i++) {
        ...
        const bussData: BusinessData = {
            ...
            property_rate: propertyRates[propertyClass as keyof typeof propertyRates] || 100, 
...
function findTotalGrade(totalmarks: number): string {
...
function getStrategicLocationMarks(location: string): number {
    const marksMap = {
        'Excellent': 5,
        'Very good': 4,
        'Good': 3,
...
function getRatingMarks(rating: number): number {
    return rating || 0;
}
...
// Function to calculate total marks
function calculateTotalMarks(data: BusinessData): number {
    ...
    const isLocationKey = (key: string): key is keyof typeof locationValues => {
