...
router.post('/', async (req, res) => {
    ...
    try {
        const [rows] = await connection.execute('SELECT * FROM tb_BudgetAssess WHERE month = ? AND fiscalyear = ?', [budgetAssessData.month, budgetAssessData.fiscalyear]);
...
router.put('/:month/:fiscalyear', async (req, res) => {
    ...
    try {
        ...
        const [result] = await connection.execute(`UPDATE tb_BudgetAssess SET budget = ?, amount = ?, variance = ?, assessmentby = ? 
