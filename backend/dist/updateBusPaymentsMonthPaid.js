...
async function updateBusPaymentsMonthPaid() {
    const client = await pool.connect();
    try {
        await client.query('BEGIN'); // Start a transaction
        // Retrieve all buspayments where monthpaid is currently "February"
        const busPaymentsResult = await client.query('SELECT buss_no FROM buspayments WHERE monthpaid = $1', ['February']);
        ...
        for (const payment of busPayments) {
            ...
            await client.query('UPDATE buspayments SET monthpaid = $1 WHERE buss_no = $2', [randomMonth, payment.buss_no]);
        }
...
updateBusPaymentsMonthPaid()
    .then(console.log)
    .catch(console.error);
//# sourceMappingURL=updateBusPaymentsMonthPaid.js.map