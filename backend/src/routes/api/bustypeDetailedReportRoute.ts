...
// Create
router.post('/', async (req: Request<{}, {}, BusTypeDetailedReport>, res: Response) => {
...
// Update
router.put('/:buss_no', async (req: Request<{ buss_no: string }, {}, BusTypeDetailedReport>, res: Response) => {
    const buss_no = parseInt(req.params.buss_no);
    const report: BusTypeDetailedReport = req.body;
    const query = `
...
router.get('/:zone/:businessType/:fiscalyear', async (req: Request<{ zone: string, businessType: string, fiscalyear: number }>, res: Response) => {
...
console.log('about to delete')
        ...
        const businessesResult = await client.query('SELECT * FROM business WHERE  status = $1 AND buss_type = $2 AND electroral_area = $3 ORDER BY electroral_area ASC',
         ['Active', businessType, zone]
        );
console.log('after delete')
        // Access the rows array
        const businesses: Business[] = businessesResult.rows; // Assuming Business is your interface for the business records
