...
router.post('/processOperatingPermits/:electoral_area/:fiscal_year', async (req, res) => {
    ...
    const connection = await mysql.createConnection(dbConfig);
    let [businessRows] = await connection.execute('SELECT * FROM tb_business WHERE electoral_area = ?', [electoralarea]);
    ...
    for (let i = 0; i < businessRows.length; i++) {
        ...
        const [updateResult] = await connection.execute('UPDATE` tb_bussCurrBalance  SET balancebf = ?,  WHERE buss_no = ?  AND fiscalyear = ?', [varBalance, buss_no, fiscalyear]);
    }
    for (let i = 0; i < businessRows.length; i++) {
        const { buss_no: varRecno } = businessRows[i].buss_no;
        let [busiUpdateRow] = await connection.execute('SELECT * FROM tb_bussCurrBalance WHERE buss_no = ? AND fiscalyear = ?', [varRecno, fiscalyear]);
        ...
        if (busiUpdateRow.length > 0) {
            await connection.execute('UPDATE tb_business SET current_rate = ? WHERE buss_no = ?', [busiUpdateRow[0].current_rate, busiUpdateRow[0].buss_no]);
            let varBalBf = findBusinessBalance(busiUpdateRow[0].buss_no);
            let varYear = new Date().getFullYear();
            await connection.execute('UPDATE tb_business SET balancenew = ? WHERE buss_no = ?', [varBalBf, busiUpdateRow[0].buss_no]);
        }
    }
    ...
    if (electoralarea) {
        const [electoralAreaResult] = await connection.execute('SELECT * FROM tb_electoral_area WHERE electoral_area = ?', [electoralarea]);
    }
    else {
    ...
    await connection.execute(`INSERT INTO tmp_business 
        ...
        AND status = "Active" 
        AND balance > 0 
        ORDER BY buss_name ASC`, [electoralarea]);
    const [recReport] = await connection.execute(`SELECT DISTINCT * FROM tb_BussCurrBalance WHERE fiscalyear = ? AND electoral_area = ?`, [fiscalyear, electoralarea]);
    if (recReport.length == 0) {
        res.status(404).json({ message: 'No paid bills found for the electoral area' });
        return;
    }
    const [tmpBussCurrBalanceRows] = await connection.execute(`INSERT INTO tmp_BussCurrBalance SELECT * FROM tb_BussCurrBalance WHERE fiscalyear = ? AND electoral_area = ?`, [fiscalyear, electoralarea]);
    // Add serial numbers
