    ...
    try {
        ...
        for (const row of result.rows) {
            ...
            if (bussType === 'All business types') {
                ...
                recSumm = await client.query('SELECT SUM(current_balance) AS totsum FROM busscurrbalance WHERE fiscalyear = $1 AND electoralarea ILIKE $2', [thisYear, row.electroral_area]);
                ...
                recSumm = await client.query('SELECT SUM(current_balance) AS totsum FROM busscurrbalance WHERE fiscalyear = $1 AND buss_type ILIKE $2 AND electoralarea ILIKE $3', [thisYear, bussType, row.electroral_area]);
