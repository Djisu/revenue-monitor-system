...
router.post('/', async (req, res) => {
    ...
    try {
        const [rows] = await connection.execute('SELECT * FROM tb_transSavings WHERE buss_no = ?  AND transdate = ?', [transSavingsData.buss_no, transSavingsData.transdate]);
        if (Array.isArray(rows) && rows.length > 0) {
            res.status(404).json({ message: 'Transaction Savings record not found' });
            return;
        }
        // Insert the new transaction savings data
        const [result] = await connection.execute(`INSERT INTO tb_transSavings 
...
router.get('/:buss_no/:transdate', async (req, res) => {
    ...
    const connection = await mysql.createConnection(dbConfig);
    try {
        const [rows] = await connection.execute('SELECT * FROM tb_transSavings WHERE buss_no = ?  AND transdate = ?', [buss_no, transdate]);
        if (Array.isArray(rows) && rows.length == 0) {
            res.status(404).json({ message: 'Transaction Savings record not found' });
            return;
        }
...
router.put('/:buss_no/:transdate', async (req, res) => {
    ...
    try {
        ...
        const [result] = await connection.execute(`UPDATE tb_transSavings 
...
router.delete('/:buss_no/:transdate', async (req, res) => {
    ...
    try {
        ...
        const [result] = await connection.execute('DELETE FROM tb_transSavings WHERE buss_no = ? AND transdate = ?', [buss_no, transdate]);
