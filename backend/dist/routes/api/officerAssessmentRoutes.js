...
router.post('/', async (req, res) => {
    ...
    try {
        const { rows } = await pool.query('SELECT * FROM officerassessment WHERE officer_no = $1 AND bus_year = $2', [officerAssessmentData.officer_no, officerAssessmentData.bus_year]);
        if (rows.length > 0) {
            res.status(409).json({ message: 'Officer assessment record already exists' });
            return;
        }
        const { rows: result } = await pool.query(`INSERT INTO officerassessment 
...
router.get('/:officer_no/:bus_year', async (req, res) => {
    ...
    try {
        const { rows } = await pool.query('SELECT * FROM officerassessment WHERE officer_no = $1 AND bus_year = $2', [officer_no, bus_year]);
        if (rows.length == 0) {
