...
const getAllPhotos = async () => {
    ...
    try {
        ...
        return rows.map((row) => Buffer.from(row.photo));
    }
    catch (err) {
        throw err;
    }
...
router.post('/store', upload.single('photo'), async (req, res) => {
    try {
        ...
        try {
            ...
            const [results] = await connection.execute('SELECT * FROM tb_photos WHERE officer_no = ?', [officer_no]);
            if (results.length > 0) {
            ...
            const [insertResult] = await connection.execute('INSERT INTO tb_photos (officer_no, photo, photo_name, photo_type) VALUES (?, ?, ?, ?)', [officer_no, photoBuffer, photo_name, photo_type]);
