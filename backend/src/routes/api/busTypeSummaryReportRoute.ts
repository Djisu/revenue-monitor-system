    ...
    try {
        ...
        for (const row of result.rows) {
            ...
            if (bussType === 'All business types') {
                ...
                let recSumm = await client.query('SELECT SUM(current_balance) AS totsum FROM busscurrbalance WHERE fiscalyear = $1 AND buss_type ILIKE $2 AND electoralarea ILIKE $3', 
            ...
            if (bussType === 'All business types') {
                ...
                console.log('specific business type')
                let recSumm = await client.query('SELECT SUM(paidamount) AS totpayments FROM buspayments WHERE fiscal_year = $1 AND buss_type ILIKE $2 AND electroral_area ILIKE $3', 
                   [thisYear, bussType, row.electroral_area]
                );
                varPayment = recSumm.rows.length > 0 ? parseFloat(recSumm.rows[0].totpayments) : 0;
            }
