...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const accReceipt = await pool.query('SELECT * FROM tb_AccReceipt WHERE batchno = $1 AND fiscalyear = $2', [accReceiptData.batchno, accReceiptData.fiscalyear]);
        ...
        const result = await pool.query(`INSERT INTO tb_AccReceipt (fiscalyear, batchno, firstno, lastno) 
            VALUES ($1, $2, $3, $4)`, [
...
router.get('/:batchno/:fiscalyear', async (req, res) => {
    ...
    try {
        // Check if an operator permission with the same OperatorID already exists
        const accReceipt = await pool.query('SELECT * FROM tb_AccReceipt WHERE batchno = $1 AND fiscalyear = $2', [batchno, fiscalyear]);
        if (accReceipt.rows.length == 0) {
...
router.put('/:batchno/:fiscalyear', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(`UPDATE tb_AccReceipt SET fiscalyear = $1, firstno = $2, lastno = $3 
