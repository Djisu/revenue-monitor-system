...
const getAllPhotos = async () => {
    ...
    const sql = 'SELECT officer_no, photo FROM tb_photos';
    try {
...
const getPhoto = async (officer_no) => {
    const client = await pool.connect();
    const sql = 'SELECT photo FROM tb_photos WHERE officer_no = $1';
    try {
...
const deletePhoto = async (officer_no) => {
    const client = await pool.connect();
    const sql = 'DELETE FROM tb_photos WHERE officer_no = $1';
    try {
        const result = await client.query(sql, [officer_no]);
        return result;
    }
...
router.post('/store', upload.single('photo'), async (req, res) => {
    try {
        ...
        const checkQuery = 'SELECT * FROM tb_photos WHERE officer_no = $1';
        ...
        const insertQuery = 'INSERT INTO tb_photos (officer_no, photo, photo_name, photo_type) VALUES ($1, $2, $3, $4)';
...
router.get('/retrieve/:officer_no', async (req, res) => {
    ...
    try {
        ...
        const sql = 'SELECT photo, photo_name, photo_type FROM tb_photos WHERE officer_no = $1';
        try {
