...
// Create
router.post('/create/:firstDate/:lastDate/:zone/:bussType', async (req: Request, res: Response) => {
    try {
        ...
        try {
            ...
            const result: QueryResult<Business> = await client.query(
                `SELECT DISTINCT electroral_area, buss_type FROM businesses WHERE status = $1  AND electroral_area = $1 GROUP BY electroral_area, buss_type`,
                ['Active', bussType, zone]
            );
           ...
           let varPayment = 0;
            ...
            if (result.rows.length === 1) {       
                try {                  
                    ...
                    if (zone.length > 0) {
                        recSumm =  await client.query('SELECT SUM(amount) AS totPayments FROM tb_busPayments WHERE fiscal_year = $1 AND buss_type = $2 AND electoral_area = $3',
                        [thisYear, bussType, zone]);
                    } 
                    
                    if (zone.length === 0){
                        recSumm =  await client.query('SELECT SUM(amount) AS totPayments FROM buspayments WHERE fiscal_year = $1 AND buss_type = $2',
                        [thisYear, bussType]);
                    }
            ...
            if (result.rows.length > 1) {
               ...
               for (const row of result.rows) {
                   ...
                   if (zone.length > 0) {
                       recSumm2 =  await client.query('SELECT SUM(current_balance) AS totSum FROM busscurrbalance WHERE fiscalyear = $1 AND buss_type = $2 AND electoralarea = $3',
                       [thisYear, bussType, row.electroral_area]);
                   } 

                   if (zone.length === 0){
                       recSumm2 =  await client.query('SELECT SUM(current_balance) AS totSum FROM busscurrbalance WHERE fiscalyear = $1 AND buss_type = $2',
                       [thisYear, bussType]);
                   }
                   ...
                   if (zone.length > 0) {
                       recSumm2 =  await client.query('SELECT SUM(amount) AS totPayments FROM buspayments WHERE fiscal_year = $1 AND buss_type = $2 AND electoral_area = $3',
                       [thisYear, bussType, row.electroral_area]);
                   } 
                    
                    if (zone.length === 0){
                        recSumm2 =  await client.query('SELECT SUM(amount) AS totPayments FROM buspayments WHERE fiscal_year = $1 AND buss_type = $2',
                        [thisYear, bussType]);
                    }
            ...
            const busTypeSummaryReport: QueryResult<BusTypeSummaryReport> = await client.query(
