...
router.post('/', async (req, res) => {
    const propertyOfficerAssessmentData = req.body;
    const connection = await mysql.createConnection(dbConfig);
    try {
        // Insert the new property officer assessment data
        const [result] = await connection.execute(`INSERT INTO tb_PropertyOfficerAssessment 
...
router.get('/:officer_no/:bus_year', async (req, res) => {
    ...
    const connection = await mysql.createConnection(dbConfig);
    try {
        const [rows] = await connection.execute('SELECT * FROM tb_PropertyOfficerAssessment WHERE officer_no = ? AND bus_year = ?', [officer_no, bus_year]);
...
router.put('/:officer_no/:bus_year', async (req, res) => {
    ...
    try {
        ...
        const [result] = await connection.execute(`UPDATE tb_PropertyOfficerAssessment 
