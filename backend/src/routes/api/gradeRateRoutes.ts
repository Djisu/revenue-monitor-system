...
router.post('/', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        [gradeRateData.grade, gradeRateData.minValuex, gradeRateData.maxValuex]);
...
router.get('/:grade/:minValuex/:maxValuex', async (req: Request, res: Response) => {
    ...
    try {
        const [result] = await connection.execute('SELECT * FROM tb_graderate WHERE grade = ? AND minValuex = ? AND maxValuex = ?', 
...
router.put('/:grade/:minValuex/:maxValuex', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const [result] = await connection.execute(
            `UPDATE tb_graderate SET minValuex = ?, maxValuex = ?, rate = ? 
            WHERE grade = ?`,
            [
                gradeRateData.minValuex,
                gradeRateData.maxValuex,
...
router.delete('/:grade/:minValuex/:maxValuex', async (req: Request, res: Response) => {
    const { grade, minValuex, maxValuex } = req.params;
