...
router.post('/create', async (req, res) => {
    ...
    const businessTypeData = req.body;
    const connection = await mysql.createConnection(dbConfig);
    try {
        const [rows] = await connection.execute('SELECT * FROM tb_BusinessType WHERE Business_Type = ?', [businessTypeData.Business_Type]);
        if (Array.isArray(rows) && rows.length > 0) {
            res.status(409).json({ message: 'Business Type record already exists.' });
            return;
        }
        // Insert the new BusinessType data
        const [result] = await connection.execute(`INSERT INTO tb_BusinessType (Business_Type) 
            VALUES (?)`, [businessTypeData.Business_Type]);
...
router.put('/:Business_Type', async (req, res) => {
    ...
    try {
        ...
        const [result] = await connection.execute(`UPDATE tb_BusinessType SET Business_Type = ? 
            WHERE Business_Type = ?`, [
