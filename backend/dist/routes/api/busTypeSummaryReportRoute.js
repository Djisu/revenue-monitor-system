...
router.get('/create/:firstDate/:lastDate/:zone/:bussType/:user', async (req, res) => {
    ...
    try {
        ...
        const searchResult = await client.query('SELECT * FROM bustypesummaryreport WHERE user_id = $1', [userId]);
        ...
        for (const row of result.rows) {
            ...
            if (bussType === 'All business types') {
                ...
                const recSumm = await client.query('SELECT SUM(current_balance) AS totsum FROM busscurrbalance WHERE fiscalyear = $1 AND electoralarea ILIKE $2', [thisYear, row.electroral_area]);
                varCurrRate = recSumm.rows.length > 0 ? parseFloat(recSumm.rows[0].totsum) : 0;
            }
            else {
                console.log('specific business type');
                const recSumm = await client.query('SELECT SUM(current_balance) AS totsum FROM busscurrbalance WHERE fiscalyear = $1 AND buss_type ILIKE $2 AND electoralarea ILIKE $3', [thisYear, bussType, row.electroral_area]);
                varCurrRate = recSumm.rows.length > 0 ? parseFloat(recSumm.rows[0].totsum) : 0;
            }
            ...
            if (bussType === 'All business types') {
                console.log('All business types');
                const recSumm = await client.query('SELECT SUM(paidamount) AS totpayments FROM buspayments WHERE fiscal_year = $1 AND electroral_area ILIKE $2', [thisYear, row.electroral_area]);
                varPayment = recSumm.rows.length > 0 ? parseFloat(recSumm.rows[0].totpayments) : 0;
            }
            else {
                console.log('specific business type');
                const recSumm = await client.query('SELECT SUM(paidamount) AS totpayments FROM buspayments WHERE fiscal_year = $1 AND buss_type ILIKE $2 AND electroral_area ILIKE $3', [thisYear, bussType, row.electroral_area]);
                varPayment = recSumm.rows.length > 0 ? parseFloat(recSumm.rows[0].totpayments) : 0;
            }
        ...
        // Fetching the report
        const results = await client.query(`SELECT DISTINCT * FROM bustypesummaryreport WHERE buss_type = $1 AND user_id = $2`, [bussType, userId]);
        // Return the response
...
router.put('/update/:firstDate/:lastDate/:zone/:bussType', async (req, res) => {
    try {
        ...
        try {
            ...
            const businessTypeSummaryReport = results.rows;
...
router.delete('/delete/:firstDate/:lastDate/:zone/:bussType', async (req, res) => {
    try {
        ...
        try {
            ...
            await client.query('DELETE FROM bustypesummaryreport WHERE buss_type = $1 AND transdate >= $2 AND transdate <= $3 AND electoral_area = $4', [bussType, firstDate, lastDate, zone]);
            // Select from bustypesummaryreport table
            const result = await client.query(`SELECT * FROM bustypesummaryreport WHERE buss_type = $1 AND transdate >= $2 AND transdate <= $3 AND electoral_area = $4`, [bussType, firstDate, lastDate, zone]);
            const businessTypeSummaryReport = result.rows;
            if (businessTypeSummaryReport.length > 0) {
                console.log('BusTypeSummaryReport fetched');
                return res.status(200).json({ message: 'BusTypeSummaryReport fetched', data: businessTypeSummaryReport });
            }
...
router.get('/read/:firstDate/:lastDate/:zone/:bussType', async (req, res) => {
    try {
        ...
        try {
            // Select from bustypesummaryreport table
            const result = await client.query(`SELECT * FROM bustypesummaryreport WHERE buss_type = $1 AND transdate >= $2 AND transdate <= $3 AND electoral_area = $4`, [bussType, firstDate, lastDate, zone]);
            const busTypeSummaryReport = result.rows;
