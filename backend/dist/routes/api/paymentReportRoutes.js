...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const paymentReportData = req.body;
        const [rows] = await connection.execute('SELECT * FROM tb_PaymentReport WHERE buss_no = ? AND fiscalyear = ?', [paymentReportData.buss_no, paymentReportData.fiscalyear]);
        if (Array.isArray(rows) && rows.length > 0) {
            res.status(409).json({ message: 'Payment report record already exists' });
            return;
        }
        const [result] = await connection.execute(`INSERT INTO tb_PaymentReport 
...
router.get('/:buss_no/:fiscalyear', async (req, res) => {
    ...
    const connection = await mysql.createConnection(dbConfig);
    try {
        const [result] = await connection.execute('SELECT * FROM tb_PaymentReport WHERE buss_no = ? AND fiscalyear = ?', [buss_no, fiscalyear]);
...
router.put('/:buss_no/:fiscalyear', async (req, res) => {
    ...
    const connection = await mysql.createConnection(dbConfig);
    try {
        const [row] = await connection.execute('SELECT * FROM tb_PaymentReport WHERE buss_no = ? AND fiscalyear = ?', [buss_no, fiscalyear]);
        if (Array.isArray(row) && row.length == 0) {
            res.status(409).json({ message: 'Payment report record does not exist' });
            return;
        }
        // Update the payment report data
        const [result] = await connection.execute(`UPDATE tb_PaymentReport 
